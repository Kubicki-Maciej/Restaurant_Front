...
export default function KitchenOrders() {
  ...
  async function fetchData() {
    client
      .get(`/kitchen/get_orders/`)
      .then((actualData) => {
        setData("actualData.data");
        setData(actualData.data);
        setOrderDone(
          actualData.data.filter((order) => order.order_status == "DONE")
        );
        setOrderInProggres(
          actualData.data.filter((order) => order.order_status == "IN_PROGRESS")
        );
        setOrderWatting(
          actualData.data.filter((order) => order.order_status == "WAITING")
        );
      })
      .catch((err) => {
  ...
  const handleDragEnd = (result) => {
    ...
    const order = findItemById(draggableId, [
      ...orderDone,
      ...orderInProggres,
      ...orderInWatting,
    ]);
      ...
      <div className="KitchenOrders">
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          ...
          >
            ...
            <KitchenOrdersLists
              title={"IN_PROGRESS"}
              orders={orderInProggres}
              id={"2"}
            />
            <KitchenOrdersLists
              title={"WAITTING"}
              orders={orderInWatting}
              id={"3"}
            />
          </div>
        </DragDropContext>
        <OrderCount
          waitingOrders={orderInWatting}
          inProgressOrders={orderInProggres}
        ></OrderCount>
      </div>
    );
  } else {
